export const webDevDataMapper = data => {
  return {
    ...
    study_programs: [
      {
        program_package: data.program_package_includes.map(
          el => el.program_package_title
        ),
        terms_and_documents: data.terms_and_documents_for_registration,
        scholarship_opportunities: data.scholarship_opportunities.map(
          el => el.scholarship_opportunities_title
        ),
        ...
        courses: data.study_program.map(program => {
          return {
            ...
            course_progression:
              program.study_program_more[0].course_progression.map(
                el => el.course_progression_item
              ),
